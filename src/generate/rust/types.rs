...
fn field_name(s: &str) -> Result<syn::Ident, Error> {
    let ident = s.replace("@", "at_").to_snake_case();
    syn::parse_str(&ident)
        .or_else(|_| syn::parse_str(&format!("r#{}", ident)))
        .with_context(|| SynError { token: s.to_string() })
}

fn type_name(s: &str) -> Result<syn::Type, Error> {
    syn::parse_str(&s).with_context(|| SynError { token: s.to_string() })
}
...
fn schema_kind(i: &openapiv3::SchemaKind, source: &openapiv3::OpenAPI) -> Result<String, Error> {
    Ok(match i {
        ...
        x => return Err(Error::Unimplemented { info: format!("schema type {:?}", x) }),
    })
}
    ...
    let res = schemas
        ...
        .flat_map(|(struct_name, schema)| {
            ...
            match &schema.data.schema_kind {
                ...
                openapiv3::SchemaKind::Type(x) => {
                    Err(Error::Unimplemented { info: format!("schema type {:?}", x) })
                }
                x => Err(Error::Unimplemented { info: format!("schema type {:?}", x) }),
            }
        })
        .collect::<TokenStream>();
